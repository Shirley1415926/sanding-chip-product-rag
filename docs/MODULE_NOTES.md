...
解决什么：检索命中正确并不代表模型最终回答忠实、完整或会遵守转人工边界；该模块把“最终生成”与已有检索评估分开检验。
输入 → 输出：26 道独立题 + Dense Top-K 证据 + 本地 `.env` 中显式配置的真实 OpenAI Chat Completions 兼容模型 → 逐题答案、是否转人工、来源、模型名、耗时、五个维度的自动检查与人工复核工作清单。
为什么：自动部分使用可重复规则，仅依据问题、检索上下文、答案和预期要点，避免由隐藏常识的 Judge 替代证据；同时承认规则和 LLM Judge 都不是绝对真相。脚本强制把 API 调用与普通单测/CI 隔离，硬安全门仍在 LLM 前执行。所有自动失败题和至少 20% 自动通过题会被确定性抽取为人工复核项；在复核完成前，报告不会声称达到独立演示门槛。
