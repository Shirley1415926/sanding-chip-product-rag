...
真实生成评估脚本只从本地 `.env` 读取 `LLM_API_BASE`、`LLM_API_KEY` 和 `LLM_MODEL`，且必须显式执行，不能由单元测试或 CI 触发。它固定验证 Dense 与 0.60 阈值；对每题记录答案、完整 Top-K `retrieval_sources`、经模型引用和后端验证的 `returned_sources`、模型名、耗时、硬安全门和是否到达 LLM 边界。每条传入模型的 `S` 证据只包含公开 `商品名称` 和 Markdown 证据正文，绝不包含 `product_id`、chunk ID 或分数；自动忠实度评估使用同一份公开商品名加正文证据。提示要求展示价问题只回答有直接证据的商品名与展示价，选品或识别问题首句说出完整公开商品名。实际运行的原始回答与 trace 默认写入 gitignore 的 `data/runtime/`；`--prepare-only` 才更新无原始回答的透明 docs 报告。模型生成的无效 JSON 或无效来源 ID 是可评估的单题 badcase；API、网络、超时、认证或提供方响应层的运行异常会在首次发生时 fail-fast，以非 0 状态写入 `status=aborted` 的本地诊断，绝不写成 `completed` 或生成质量结论。自动检查仅基于问题、检索证据、最终回答与预期要点，分别报告忠实度、相关性、完整性、来源正确性和安全合规；它不调用 LLM Judge，也不是绝对真相。对于既有已完成运行，`scripts/recalibrate_generation_evaluation.py` 只读取父 trace、冻结题集和公开 Markdown metadata，生成带父 trace SHA-256 的派生报告；不读取 `.env`、不初始化 Embedding、不导入或调用 LLM、也不发起网络请求。它重算 20% 人工抽样并仅复用重叠题目的既有复核。第三轮本地 DeepSeek 父运行经该口径校准后为自动 26/26、重算人工抽样 4/4，满足限定目录与固定题集下的独立演示门槛；这一结果不是正式商用结论。九道硬安全门题由独立安全测试保证，不进入生成答案的人工抽样。`scripts/apply_generation_manual_review.py` 只读写同一轮本地 trace/report，且仅接受脚本已选中的 case，复算人工统计与演示门槛。模型回答在写入本地追踪前会掩盖常见 API Key、邮箱和手机号模式。详见 `docs/GENERATION_EVALUATION_REPORT.md`。
